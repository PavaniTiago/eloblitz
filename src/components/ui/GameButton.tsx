...
export default function GameButton({ icon, title, className, onClick }: GameButtonProps) {
    return (
        <button onClick={onClick} className={cn(`flex flex-col items-center justify-center hover:scale-110 hover:text-white text-xl text-primary hover:text-white font-bold py-2 px-4 rounded z-20 transition-all ${className}`)}>
