      ...
      <section className="h-screen flex flex-col items-center justify-center absolute z-20">
        ...
        <section className="h-full flex flex-col items-center justify-center mt-12 relative">
          ...
          <div className="w-screen">
            <Image alt={`${game == "valorant" ? "raze image" : "lee sin image"}`} src={`${game == "valorant" ? "/raze.svg" : "/lee-sin.svg"}`} width={0} height={0} className={`saturate-150 brightness-110 absolute ${game == "valorant" ? "right-2 bottom-4 w-[30rem] h-[43rem] object-cover" : "left-2 bottom-4  w-[40rem] h-[40rem]"}`}/>
          </div>
          <div className={`flex items-center gap-10 mt-6 ${game == "valorant" ? "mr-40" : "ml-40"}`}>
